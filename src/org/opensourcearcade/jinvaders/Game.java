...
public final class Game extends Applet implements Runnable {
	...
	public Game() {
		...
		panel.addKeyListener(new KeyAdapter() {                          //captura o enter para entrar no jogo
	...
	public void init() {
		...
		imagens.setBackbuffer(
				ToolBox.convertToCompatibleImage(new BufferedImage(WIDTH, HEIGHT, BufferedImage.TYPE_INT_RGB)));
		g2d = imagens.getBackbuffer().createGraphics();
		Color cordeFundo = new Color(28, 28, 28);
		g2d.setBackground(cordeFundo);

		playerName1 = "SCORE ATUAL";
		playerName2 = "";
	...
	public void updateGame(long time) {
		...
		if (gameState == GameStates.SPLASH_SCREEN) {
			updateSplashScreen(time);
		}
		if (gameState == GameStates.HELP_SCREEN) {
			updateHelpScreen(time);
		}
		if (gameState == GameStates.HIGH_SCORE_SCREEN) {
			updateHighScoreScreen(time);
		}
		if (gameState == GameStates.IN_GAME_SCREEN) {
			updateInGameScreen(time);
		}
		if (gameState == GameStates.INPUT_NAME_SCREEN) {
			updateInputNameScreen(time);
		}
		if (gameState == GameStates.GAME_OVER_SCREEN) {
	...
	public void updateScreenUtil(GameStates gs, long time) {
		...
		if (splashScreenTimer <= 0) {
			...
			splashScreenTimer = 9999999999999L;
			gameState = gs;
		}
	}
	...
	private void updateShooting(long time) {
		Entity shot;
		if (keyboard.isSpaceKey() && keyboard.isSpaceKeyReleased() && !player.getPlayerShot().visible
				&& player.frame == 0) {
			...
			if (t - lastShotTime > 300000) {
				...
				player.getPlayerShot().sy = -(Math
						.round(10.0f * (float) Speeds.getPlayerShotSpeed() / (float) FRAMES_PER_SECOND)) / 10.0f;
			}
		}
	...
	private void updateCollisions(long time) {
		...
		// ufo ./. playershot
		if (ufo.frame < 2 && ufo.visible && player.getPlayerShot().visible
				&& ToolBox.checkCollision(player.getPlayerShot(), ufo)) {
	...
	public void paint() {
		...
		} else if (gameState == GameStates.HELP_SCREEN) {
			drawHelpScreen(g2d, fontHeight);
		}

		else if (gameState == GameStates.HIGH_SCORE_SCREEN) {
			drawHighScoreScreen(g2d, fontHeight);
		} else if (gameState == GameStates.IN_GAME_SCREEN) {
			drawIngameScreen(g2d);
		} else if (gameState == GameStates.GAME_OVER_SCREEN) {
			drawGameOverScreen(g2d, fontHeight);
		} else if (gameState == GameStates.INPUT_NAME_SCREEN) {
		...
		if (!Sound.isEnabled())
			g2d.drawImage(imagens.getSndOffImg(), WIDTH - imagens.getSndOffImg().getWidth() - 1,
					Pos.BOTTOM_LINE_POS + 2, null);
	...
	private void drawIngameScreen(Graphics g) {
		...
		if (player.getPlayerShot().visible)
			g.fillRect((int) player.getPlayerShot().x, (int) player.getPlayerShot().y, player.getPlayerShot().w,
					player.getPlayerShot().h);
